...
public class ProjectListDTO implements DTO<Project> {
			...
			if (contract.getContractEndDate() != null) {
				...
				if (opt.isPresent()) {
					if(contract.getContractEndDate().isBefore(opt.get().getSecondHalfStartDate())) {
						setContractProgressPeriod(opt.get().getPeriod() + " 上期");
					} else {
						setContractProgressPeriod(opt.get().getPeriod() + " 下期");
					}
				}
			} else {
	...
	public String getProjectClassificationColor() {
		if (this.projectCategoryId != null) {
			ProjectClassification pc = new ProjectClassificationRepository().getClassificationColor(this.projectCategoryId);
			if(pc != null) {
				return pc.getName();
			}
		}
