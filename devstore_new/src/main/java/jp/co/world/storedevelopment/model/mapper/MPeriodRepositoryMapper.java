...
public interface MPeriodRepositoryMapper extends RepositoryMapper<MPeriod> {
	...
	@Select("select id, up_down from ("
			+ "select distinct mpe.period as id, case when p.implementation_datetime between mpe.start_date and mpe.first_half_end_date then '上期' else '下期' end as up_down, case when p.implementation_datetime between mpe.start_date and mpe.first_half_end_date then 1 else 2 end as up_down_order_by from project p left join m_period mpe on p.implementation_datetime between mpe.start_date and mpe.end_date where operation_division = '進行中' and implementation_datetime is not null "
			+ " union "
			+ "select distinct mpe.period as id, case when p.implementation_schedule_datetime between mpe.start_date and mpe.first_half_end_date then '上期' else '下期' end as up_down, case when p.implementation_schedule_datetime between mpe.start_date and mpe.first_half_end_date then 1 else 2 end as up_down_order_by from project p left join m_period mpe on p.implementation_schedule_datetime between mpe.start_date and mpe.end_date where operation_division = '進行中' and implementation_schedule_datetime is not null "
