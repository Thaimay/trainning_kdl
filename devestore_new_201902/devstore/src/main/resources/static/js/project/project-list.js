...
function createPagingKeyword(){
	...
	// ページャーの連打対策
	setTimeout(function() {
		pagination.find('li').on({
			'click': function(event) {
				pagination.find('li').each(function(i, el) {
...
$(function() {
	...
	$("#j-form-update-day-to").keyup(function() {
		const value = $(this).val();
	
		if (value && value.length > 6) {
			alert("更新日は6桁以内で入力してください");
	...
	$("#buildingExpectedValueFrom").on("keyup", function(e) {
		const value = $(this).val();
		

		if ((value && parseInt(value) < 0) || value == "-") {
			alert("館期待値比は正数で入力してください");
			$(this).val("");
		}
	...
	$("#buildingExpectedValueTo").on("keyup", function(e) {
		const value = $(this).val();
	...
	$('#j-search').click(function(e) {		
		...
		const updateFrom = $("#j-form-update-from").val();
		const updateTo = $("#j-form-update-to").val();

		if (!updateFrom.match(regex)) {
			$("#j-form-update-from").val("");
		}
		
		if (!updateTo.match(regex)) {
			$("#j-form-update-to").val("");
		}
...
function childrenUnCheckChange(element) {
	let id = $(element).attr('id');
