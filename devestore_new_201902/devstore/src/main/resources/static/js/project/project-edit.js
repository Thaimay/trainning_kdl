...
//            }
            if(data.participatingStoreCorporation != undefined && data.participatingStoreCorporation != null){
				...
				projectParticipatingStoreCorporationProgressSuggest.setValue([data.participatingStoreCorporation])
            } else {
        	...
        	listBrandIncome = [];
			projectParticipatingStoreCorporationProgressSuggest.clear();
        	createProjectTitle.brand = "";
        }
    });
}
...
var setDataFromShop = function(autoRefBuildingData) {
	if(refShop){
    	...
    	if(refShopData.operationForm !== undefined && refShopData.operationForm !== null){
				$("#j-form-operation-form-label").text(refShopData.operationForm);
				
				$contractProgressOperationForm = $("#j-form-contract-progress-operation-form")
				if ($contractProgressOperationForm.val() === '') {
					$contractProgressOperationForm.val(refShopData.operationForm);  // unneed??
				}
    	} else{
    		$("#j-form-operation-form-label").text("");
    	}
...
$(function() {
	...
	$(suggestShopName).on( 'selectionchange', function() {
		...
		if (selectedData.length > 0) {
			...
			if (refShopData.rentContract != null && refShopData.rentContract.iContractType != null && refShopData.rentContract.iContractType.contractTypeName != undefined) {
				$("#project-contract-progress-edit-form").val(refShopData.rentContract.iContractType.contractTypeName);
				currentContract.form = refShopData.rentContract.iContractType.contractTypeName;
			}
			$("#project_edit_shop_name_temporary").hide();
			setTimeout(function () {
				if (projectSaleAgencyTargetProgressSuggest.getValue() != undefined || projectSaleAgencyTargetProgressSuggest.getValue() != null) {
					projectSaleAgencyTargetProgressSuggest.clear();
				}
				if (refShopData.salesAgencyTarget != null) {
					projectSaleAgencyTargetProgressSuggest.setValue([refShopData.salesAgencyTarget]);
				}
			}, 1000);
		} else {
			$("#project_edit_shop_name_temporary").show();
...
//                  projectPlanCurrent.endDate = $("#project-edit-i-sales-agency-contract-enddate-curent-label").val();
	...
	$("#project_edit_project_category").change(function(){
		...
		var $rentContractProgressTargets = ["#project_edit_rent_contract_startdate_progress", "#project_edit_rent_contract_enddate_progress", "#project_edit_rent_contract_number_of_year_progress"];
		setSamePlaceholder($rentContractProgressTargets, ''); 
...
function getDataForSectionProgressNegotiation() {
	...
	if($("#project_edit_progress_negotiation_floor").val()[$("#project_edit_progress_negotiation_floor").val().length-1] == "Ｆ"){
		projectSectionProgressNegotiation.floor = $("#project_edit_progress_negotiation_floor").val().substring(0, $("#project_edit_progress_negotiation_floor").val().length-1);
		$("#project_edit_progress_negotiation_floor").val(projectSectionProgressNegotiation.floor);
	}else{
...
function makeProjectTitle(){
	...
	if ($("#project_edit_shop_name").find(".ms-close-btn").length == 0) {
		_brand = "";
	}
