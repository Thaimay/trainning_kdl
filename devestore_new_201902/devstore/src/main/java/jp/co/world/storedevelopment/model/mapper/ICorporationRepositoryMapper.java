...
public interface ICorporationRepositoryMapper extends RepositoryMapper<ICorporation> {
	...
	@Select("select * from i_corporation where corporation_name like '%${text}%' or corporation_name like '%${zenkaku}%' or corporation_name like '%${hankaku}%' limit ${limit}")
	List<ICorporation> findByText(@Param("text") String text, @Param("hankaku") String hankaku,
			@Param("zenkaku") String zenkaku, @Param("limit") int limit);

	@Select("select * from i_corporation where corporation_name like '%${iCorporation.text}%' or corporation_name like '%${iCorporation.textZenkaku}%' or corporation_name like '%${iCorporation.textHankaku}%' limit ${limit}")
