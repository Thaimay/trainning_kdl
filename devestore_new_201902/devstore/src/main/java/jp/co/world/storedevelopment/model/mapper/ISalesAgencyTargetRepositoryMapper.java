...
public interface ISalesAgencyTargetRepositoryMapper extends RepositoryMapper<ISalesAgencyTarget> {
	...
	@Select("select * from i_sales_agency_target where sales_agency_target_name like '%${iSalesAgencyTarget.text}%' or sales_agency_target_name like '%${iSalesAgencyTarget.textZenkaku}%' or sales_agency_target_name like '%${iSalesAgencyTarget.textHankaku}%' limit ${limit}")
