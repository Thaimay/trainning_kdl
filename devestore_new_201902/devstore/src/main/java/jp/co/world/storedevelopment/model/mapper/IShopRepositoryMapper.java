...
public interface IShopRepositoryMapper extends ShopRelatedRepositoryMapper<IShop> {
	...
	@Select("select ish.* from i_shop ish inner join i_shop_admin isha on isha.shop_id = ish.shop_id inner join m_shop_type mst on mst.id = isha.shop_type_id where ish.open_date <= now() and (ish.close_date >= now() or ish.close_date is null) and (concat(ish.shop_cd,' ',ish.shop_name_zenkaku) like '%${shop}%' or concat(ish.shop_cd,' ',ish.shop_name_zenkaku_hankaku) like '%${hankaku}%') order by ish.shop_name_zenkaku_hankaku limit ${limit}")
	List<IShop> findByString(@Param("shop") String text, @Param("hankaku") String hankaku, @Param("limit") int limit);

	@Select("select ish.* from i_shop ish inner join i_shop_admin isha on isha.shop_id = ish.shop_id inner join m_shop_type mst on mst.id = isha.shop_type_id where ish.open_date <= now() and (ish.close_date >= now() or ish.close_date is null) and (concat(ish.shop_cd,' ',ish.shop_name_zenkaku_hankaku) like '%${shop}%' or concat(ish.shop_cd,' ',ish.shop_name_zenkaku_hankaku) like '%${hankaku}%') order by ish.shop_name_zenkaku_hankaku COLLATE \"ja_JP.utf8\" limit ${limit}")
	List<IShop> findByStringUtf(@Param("shop") String text, @Param("hankaku") String hankaku, @Param("limit") int limit);
