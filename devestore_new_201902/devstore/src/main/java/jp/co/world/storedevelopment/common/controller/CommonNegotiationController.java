	...
	public List<NegotiationRelationProjectDTO> findProject(@RequestBody @Valid RelatedFindFromProjectDTO dto) {
		try {
			...
			listProjectDto.forEach(x -> {
				String operationDivision = x.getOperationDivision();
				if (operationDivision.equals("中止") || operationDivision.equals("終了")) {
					dtos.add(new NegotiationRelationProjectDTO(x.getId(),
							ZenkakuUtils.toZenkaku("【" + operationDivision + "】　" + x.getTitle())));
				} else {
