...
public class Project extends ActiveModel<Project> implements ProjectTargetAlert {
	...
	private Float differenceWeekFromNow(LocalDate date) {
		...
		Temporal target = date.atTime(0, 0);
		Integer days = (int) Duration.between(target, now).toDays();

		if (days > 0) {
			Float much = days / 7F;
			return new BigDecimal(much).setScale(1, BigDecimal.ROUND_DOWN).floatValue();
		}
		return 0F;		
	}
	...
	public void checkOpeningDate(ProjectSectionProgress negotiationProgress) {
		...
		String otherProgress = contractProgressCalculation(list, negotiationProgress);
	...
	private String contractProgress(List<MProjectActionStatus> list, ProjectSectionProgress model) {
		return contractProgressCalculation(list, model);
	}

	private String contractProgressCalculation(List<MProjectActionStatus> list, ProjectSectionProgress model) {
		BigDecimal tsubo = model.getContractTsubo();
		LocalDate date = getImplementationDatetime();
		Boolean isOpeningStore = false;
		ZoneId zoneId = ZoneId.systemDefault();
		LocalDate now = LocalDate.now();
		Boolean isTsubo = !list.stream().anyMatch(v -> v.getOtherStatusCode().equals("AREA_ENTERED")) || tsubo != null;

		if (date == null) {
			date = getImplementationScheduleDatetime();
		}
	...
	private MProjectActionStatus calculateActionStatus(Long salesChannelId, ProjectSectionProgress model) {
		...
		if (model == null) {
			...
			otherProgress = contractProgress(list, model);
		}
